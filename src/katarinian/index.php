<html>
    <?php
        ...
        if (isset($_POST['name'])) {
            ...
            $query = "insert into katarinianlist (namee, bookname, date1, date1g, tamdid, date2) values ('".$name."', '".$book."', '".jdate("Y-m-d", strtotime("$date"), '', '', 'en')."', '".$dayte."', 1, NULL)";
        ...
        if (isset($_POST['id'])) {
            $id = $_POST['id'];
            $date = date("Y/m/d");
            $quer = "update katarinianlist set date2='".jdate("Y-m-d", strtotime("$date"), '', '', 'en')."' where id='".$id."'";
        ...
        if (isset($_POST['tamdid'])) {
            $tamdid = $_POST['tamdid'];
            $quer = "update katarinianlist set tamdid=tamdid+1 where id='".$tamdid."'";
            if (!mysqli_query($db , $quer)) {
        ...
        if (isset($_POST['del'])) {
            $del = $_POST['del'];
            $quer = "DELETE FROM katarinianlist WHERE id = '".$del."'";
            if (!mysqli_query($db , $quer)) {
        ...
        if (isset($_GET['search'])) {
            ...
            $result = mysqli_query($db, "SELECT * FROM katarinianlist WHERE namee LIKE '%".$squery."%'");
            if (!$result) {
                echo "<p>Search Query Error: " . mysqli_error($db) . "</p>";
            }
        }
        else {
            $result = mysqli_query($db, 'SELECT * FROM katarinianlist');
            if (!$result) {
    ...
    <head>
        ...
        <title>فهرست کتابخانه کاتارینیان</title>
    ...
    <body onload="$('.modal').modal();fade();tost()">
        ...
        <nav class="teal z-depth-3 darken-2">
            <div class="nav-wrapper" style="text-align:center">
                <a class="title tt center-aligned">دبیرستان کاتارینیان</a>
            ...
            <?php
                if ($result)  {
                    ...
                    for($i = 0; $i < $rows; $i++) {
                        ...
                        echo '<td><form action="http://localhost:8080/katarinian/" method="post"><input type="text" name="tamdid" value="'.$row[0].'" style="display:none"><button type="submit" class="btn-flat waves-effect waves-orange" style="width:40px;height:40px;padding:0;border-radius:500px"><i class="material-icons" style="color:#ff7700">access_time</i></button></form></td>';
                        if(is_null($row[6])) {
                            echo'<td><form action="http://localhost:8080/katarinian/" method="post"><input type="text" name="id" value="'.$row[0].'" style="display:none"><button type="submit" class="btn-flat waves-effect waves-green" style="width:40px;height:40px;padding:0;border-radius:500px"><i class="material-icons" style="color:green">add</i></button></form></td>';
                        }
                        else {
                            echo '<td>'.$row[6].'</td>';
                        }
                        echo'<td><form action="http://localhost:8080/katarinian/" method="post"><input type="text" name="del" value="'.$row[0].'" style="display:none"><button type="submit" class="btn-flat waves-effect waves-red" style="width:40px;height:40px;padding:0;border-radius:500px"><i class="material-icons" style="color:red">clear</i></button></form></td>';
                        echo '</tr>';
                    };
                }
            ?>
